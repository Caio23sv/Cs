...
local function createESP(player)
    ...
    -- Criar Highlight (aura azul nos corpos)
    ...
    highlight.OutlineColor = Color3.fromRGB(0, 150, 255) -- Aura azul
    ...
    billboard.Adornee = character:FindFirstChild("HumanoidRootPart") or character:FindFirstChild("Head") or character:WaitForChild("HumanoidRootPart", 1)
        ...
        if player and player.Character and objects.Highlight then
            ...
            if humanoid and humanoid.Health > 0 and rootPart then
                ...
                if objects.HealthLabel then
                    objects.HealthLabel.Text = string.format("Vida: %d/%d", math.floor(humanoid.Health), math.floor(humanoid.MaxHealth))
                    
                    -- Mudar cor da vida baseado na porcentagem
                    local healthPercent = humanoid.Health / humanoid.MaxHealth
                    if healthPercent > 0.7 then
                        objects.HealthLabel.TextColor3 = Color3.fromRGB(50, 255, 50)
                    elseif healthPercent > 0.3 then
                        objects.HealthLabel.TextColor3 = Color3.fromRGB(255, 255, 50)
                    else
                        objects.HealthLabel.TextColor3 = Color3.fromRGB(255, 50, 50)
                    end
                end
                ...
                if objects.DistanceLabel and localRoot then
                    ...
                    objects.DistanceLabel.Text = string.format("Distância: %d studs", math.floor(distance))
                    
                    -- Mudar cor baseado na distância
                    if distance < 20 then
                        objects.DistanceLabel.TextColor3 = Color3.fromRGB(255, 50, 50)
                    elseif distance < 50 then
                        objects.DistanceLabel.TextColor3 = Color3.fromRGB(255, 255, 50)
                    else
                        objects.DistanceLabel.TextColor3 = Color3.fromRGB(50, 255, 50)
                    end
                end
                
                -- Atualizar cor do nome baseado no time
                if objects.NameLabel then
                    if player.Team then
                        objects.NameLabel.TextColor3 = player.TeamColor.Color
                    else
                        objects.NameLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
                    end
...
local function toggleESP()
    ...
    if espEnabled then
        ...
        warn("[CAI HUB] ESP ativado! (Aura azul + Nome/Vida/Distância)")
    else
        -- Desativar ESP
