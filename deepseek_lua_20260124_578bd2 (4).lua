...
-- ESP MELHORADO COM AURA FORTE E INFORMAÇÕES EM AZUL
local espEnabled = false
local espObjects = {}
local ESP_COLOR = Color3.fromRGB(0, 150, 255) -- Azul forte para tudo
...
local function createESP(player)
    ...
    -- Criar Highlight (aura azul FORTE nos corpos)
    local highlight = Instance.new("Highlight")
    highlight.Name = "ESP_Highlight_" .. player.Name
    highlight.Adornee = character
    highlight.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop
    highlight.FillColor = Color3.fromRGB(0, 0, 0)
    highlight.FillTransparency = 0.95 -- Quase transparente
    highlight.OutlineColor = ESP_COLOR -- Aura azul FORTE
    highlight.OutlineTransparency = 0.1 -- Quase opaco (muito visível)
    highlight.Enabled = true
    highlight.Parent = game:GetService("CoreGui")
    
    -- Criar Billboard para nome, vida e distância
    ...
    -- Layout para organizar os textos
    local layout = Instance.new("UIListLayout")
    layout.Padding = UDim.new(0, 2)
    layout.SortOrder = Enum.SortOrder.LayoutOrder
    layout.Parent = container
    
    -- Nome do jogador (grande e em azul)
    local nameLabel = Instance.new("TextLabel")
    nameLabel.Size = UDim2.new(1, 0, 0, 25)
    nameLabel.BackgroundTransparency = 1
    nameLabel.Text = player.Name
    nameLabel.TextColor3 = ESP_COLOR -- Azul forte
    ...
    -- Vida do jogador (em azul)
    local healthLabel = Instance.new("TextLabel")
    healthLabel.Size = UDim2.new(1, 0, 0, 20)
    healthLabel.BackgroundTransparency = 1
    healthLabel.TextColor3 = ESP_COLOR -- Azul forte
    ...
    -- Distância (em metros e azul)
    local distanceLabel = Instance.new("TextLabel")
    distanceLabel.Size = UDim2.new(1, 0, 0, 20)
    distanceLabel.BackgroundTransparency = 1
    distanceLabel.TextColor3 = ESP_COLOR -- Azul forte
...
local function updateESP()
    ...
    for player, objects in pairs(espObjects) do
        if player and player.Character and objects.Highlight then
            local character = player.Character
            local humanoid = character:FindFirstChild("Humanoid")
            local rootPart = character:FindFirstChild("HumanoidRootPart")
            
            if humanoid and humanoid.Health > 0 and rootPart then
                -- Atualizar Highlight (aura azul forte)
                objects.Highlight.Adornee = character
                objects.Highlight.Enabled = true
                
                -- Tornar a aura MAIS FORTE (menos transparente)
                objects.Highlight.OutlineTransparency = 0.05 -- Quase opaco
                objects.Highlight.FillTransparency = 0.9 -- Fundo quase transparente
                
                -- Atualizar posição do Billboard
                if objects.Billboard and objects.Billboard.Adornee ~= rootPart then
                ...
                if objects.NameLabel then
                    ...
                    objects.NameLabel.TextColor3 = ESP_COLOR -- Azul forte
                end
                
                if objects.HealthLabel then
                    objects.HealthLabel.Text = string.format("Vida: %d/%d HP", math.floor(humanoid.Health), math.floor(humanoid.MaxHealth))
                    objects.HealthLabel.TextColor3 = ESP_COLOR -- Azul forte
                end
                
                -- Atualizar distância em metros
                if objects.DistanceLabel and localRoot then
                    local distance = (rootPart.Position - localRoot.Position).Magnitude
                    objects.DistanceLabel.Text = string.format("Distância: %d metros", math.floor(distance))
                    objects.DistanceLabel.TextColor3 = ESP_COLOR -- Azul forte
                end
            else
                removeESP(player)
            end
...
local function toggleESP()
    ...
    if espEnabled then
        ...
        warn("[CAI HUB] ESP ativado! (Aura azul forte + Nome/Vida/Distância em azul)")
    else
        -- Desativar ESP
