...
ScreenGui.Name = "CaiHub_Ultimate_V9"; ScreenGui.ResetOnSpawn = false

local MainFrame = Instance.new("Frame", ScreenGui)
MainFrame.BackgroundColor3 = Color3.fromRGB(15, 15, 18); MainFrame.AnchorPoint = Vector2.new(0.5, 0.5)
MainFrame.Position = UDim2.new(0.5, 0, 0.5, 0); MainFrame.Size = UDim2.new(0, 460, 0, 280)
MainFrame.Visible = false; MainFrame.ZIndex = 1
Instance.new("UICorner", MainFrame).CornerRadius = UDim.new(0, 10)
local MainStroke = Instance.new("UIStroke", MainFrame); MainStroke.Thickness = 2; MainStroke.Color = Color3.fromRGB(0, 255, 255)

local DragBar = Instance.new("Frame", MainFrame)
DragBar.Size = UDim2.new(1, 0, 0, 40); DragBar.BackgroundTransparency = 1; DragBar.ZIndex = 10

-- 3. DECORAÇÃO
local Title = Instance.new("TextLabel", MainFrame)
Title.Text = "CAI HUB"; Title.Position = UDim2.new(0, 48, 0, 8); Title.Size = UDim2.new(0, 150, 0, 28)
Title.TextColor3 = Color3.fromRGB(255, 255, 255); Title.Font = Enum.Font.GothamBold; Title.TextSize = 18; Title.BackgroundTransparency = 1; Title.ZIndex = 3; Title.TextXAlignment = 0

local Logo = Instance.new("Frame", MainFrame)
Logo.Size = UDim2.new(0, 28, 0, 28); Logo.Position = UDim2.new(0, 12, 0, 8); Logo.BackgroundColor3 = Color3.fromRGB(0, 255, 255); Logo.ZIndex = 3; Instance.new("UICorner", Logo).CornerRadius = UDim.new(1, 0)
local LT = Instance.new("TextLabel", Logo); LT.Text = "C"; LT.Size = UDim2.new(1,0,1,0); LT.BackgroundTransparency = 1; LT.TextColor3 = Color3.fromRGB(0,0,0); LT.Font = Enum.Font.GothamBold; LT.ZIndex = 4

local LineH = Instance.new("Frame", MainFrame); LineH.Position = UDim2.new(0, 10, 0, 42); LineH.Size = UDim2.new(0, 440, 0, 2); LineH.BackgroundColor3 = Color3.fromRGB(0, 255, 255); LineH.ZIndex = 2; LineH.BorderSizePixel = 0
local LineV = Instance.new("Frame", MainFrame); LineV.Position = UDim2.new(0, 135, 0, 50); LineV.Size = UDim2.new(0, 2, 0, 215); LineV.BackgroundColor3 = Color3.fromRGB(0, 255, 255); LineV.ZIndex = 2; LineV.BorderSizePixel = 0

-- 4. SISTEMA DE ABAS (BORDAS AJUSTADAS)
local Sidebar = Instance.new("Frame", MainFrame); Sidebar.Position = UDim2.new(0, 8, 0, 55); Sidebar.Size = UDim2.new(0, 120, 0, 215); Sidebar.BackgroundTransparency = 1; Sidebar.ZIndex = 4
Instance.new("UIListLayout", Sidebar).Padding = UDim.new(0, 5)
local PageContainer = Instance.new("Frame", MainFrame); PageContainer.Position = UDim2.new(0, 145, 0, 55); PageContainer.Size = UDim2.new(0, 305, 0, 215); PageContainer.BackgroundTransparency = 1; PageContainer.ZIndex = 4
...
local function createPage(name)
    local p = Instance.new("ScrollingFrame", PageContainer); p.Name = name; p.Size = UDim2.new(1, 0, 1, 0); p.BackgroundTransparency = 1; p.Visible = false; p.ScrollBarThickness = 0; p.ZIndex = 5
    Instance.new("UIListLayout", p).Padding = UDim.new(0, 8); Pages[name] = p; return p
end

local function addTab(name, isFirst)
    local b = Instance.new("TextButton", Sidebar); b.Size = UDim2.new(1, 0, 0, 35); b.BackgroundColor3 = Color3.fromRGB(22, 22, 25); b.Text = name; b.TextColor3 = Color3.fromRGB(150, 150, 150); b.Font = Enum.Font.GothamMedium; b.ZIndex = 6; Instance.new("UICorner", b).CornerRadius = UDim.new(0, 6)
    local s = Instance.new("UIStroke", b); s.Color = Color3.fromRGB(0, 255, 255); s.Thickness = 1.2; s.Transparency = (isFirst and 0 or 1); s.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    
    if isFirst then b.TextColor3 = Color3.fromRGB(255, 255, 255) end
    
    b.MouseButton1Click:Connect(function()
        for n, pg in pairs(Pages) do pg.Visible = (n == name) end
        for _, btn in pairs(SidebarButtons) do btn.TextColor3 = Color3.fromRGB(150, 150, 150); btn.UIStroke.Transparency = 1 end
        b.TextColor3 = Color3.fromRGB(255, 255, 255); s.Transparency = 0; lastOpenPage = name
    end)
    SidebarButtons[name] = b; return b
end
...
local function deepScan(obj, depth, path)
    ...
    for _, child in ipairs(children) do
        ...
        if string.find(nameLower, "anti") or 
           ...
           string.find(nameLower, "security") then
...
task.spawn(function()
    ...
    logScan("⚠️ Possíveis anti-cheats: "..totalAntiCheats, totalAntiCheats > 0 and Color3.fromRGB(255, 100, 100) or Color3.fromRGB(100, 255, 100))
    ...
    if totalAntiCheats > 0 then
        ...
        logScan("💡 Recomendação: Use funções com cautela!", Color3.fromRGB(255, 200, 50))
    else
